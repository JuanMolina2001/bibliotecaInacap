...
def mostrar_libros():
    ...
    consulta = "SELECT l.id_libro, l.titulo, l.autor, l.genero, COUNT(e.id_ejemplar) AS total_ejemplares, SUM(CASE WHEN e.estado = 'Disponible' THEN 1 ELSE 0 END) AS ejemplares_disponibles, SUM(CASE WHEN e.estado = 'En prestamo' THEN 1 ELSE 0 END) AS ejemplares_en_prestamo, SUM(CASE WHEN e.estado = 'Danado' THEN 1 ELSE 0 END) AS ejemplares_danados FROM libro l LEFT JOIN ejemplar e ON l.id_libro = e.id_libro GROUP BY l.id_libro;"
...
def ejemplarPrestamo(id_ejemplar):
    conexion = get_connection()
    cursor = conexion.cursor()
    consulta = "UPDATE `ejemplar` SET `estado` = 'En prestamo' WHERE `ejemplar`.`id_ejemplar` = %s;"
    valores = (id_ejemplar,)
    cursor.execute(consulta, valores)
    conexion.commit()
    cursor.close()
...
def cambiarDisponible(id_prestamo):
    conexion = get_connection()
    cursor = conexion.cursor()
    consulta = "UPDATE ejemplar AS e SET e.estado = 'disponible' WHERE e.id_ejemplar IN ( SELECT p.id_ejemplar FROM prestamo AS p WHERE p.id_prestamo = "+ str(id_prestamo) +");"
    cursor.execute(consulta,)
    conexion.commit()
    cursor.close()
    conexion.close()
