...
def ingresar_Usuario(usuario):
    ...
    consulta = "INSERT INTO `usuario` (`nombre`, `apellido`, `rut`, `tipo_usuario`, `correo`, `numero_telefono`) VALUES (?, ?, ?, ?, ?, ?);"
...
def tipoUsuario(rut):
    conexion = get_connection()
    cursor = conexion.cursor()
    consulta = "SELECT tipo_usuario from usuario WHERE rut = ?"
    valores = (rut,)
    cursor.execute(consulta, valores)
    for row in cursor:
        tipo_usuario = row[0]
...
def verificarAdmin(administrador):
    conexion = get_connection()
    cursor = conexion.cursor()
    consulta = "SELECT contrasena FROM `administrador` WHERE rut = ?;"
...
def ingresarAdmin(administrador):
    conexion = get_connection()
    cursor = conexion.cursor()
    consulta = "INSERT INTO `administrador` (`rut`, `nombre`, `apellido`, `contrasena`) VALUES (?, ?, ?, ?);"
...
def registro(time):
    conexion = get_connection()
    cursor = conexion.cursor()
    consulta = "INSERT INTO `time` (`time_id`, `hora_ingreso`, `hora_salida`, `rut_admin`) VALUES (NULL, ?, ?, ?)"
...
def verificarRut(rut):
    conexion = get_connection()
    cursor = conexion.cursor()
    consulta = "SELECT rut FROM `administrador` WHERE rut = ?;"
    valores = (rut,)
    cursor.execute(consulta,valores)
    rutConsulta = None
    for row in cursor:
